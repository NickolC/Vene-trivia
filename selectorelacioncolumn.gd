...
func _verificar_si_nivel_esta_comprado(num: int) -> bool:
	return SQLiteHelper.nivel_comprado(db, GlobalUsuario.usuario_actual_id, "columnas", num)
...
func _on_nivel_seleccionado_desde_mapa(num: int) -> void:
	...
	if GlobalUsuario.usuario_actual_id > 0:
		q_estrellas = "SELECT NU_ESTRELLAS FROM columnas_niveles WHERE NU_NIVEL = %d AND NU_USU = %d LIMIT 1;" % [num, GlobalUsuario.usuario_actual_id]
	else:
		q_estrellas = "SELECT NU_ESTRELLAS FROM columnas_niveles WHERE NU_NIVEL = %d AND NM_ALUMNO = '%s' LIMIT 1;" % [num, SQLiteHelper.escape(GlobalUsuario.nombre_alumno)]
