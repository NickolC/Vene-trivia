...
func _init():
    ...
    match operation:
        ...
        "create_scene_with_script":
            create_scene_with_script(params)
        "add_node":
            add_node(params)
        "load_sprite":
            load_sprite(params)
...
func to_res_path(path: String, default_extension: String = "") -> String:
    var normalized_path := path.strip_edges()
    if normalized_path.is_empty():
        return ""
    if not normalized_path.begins_with("res://"):
        normalized_path = "res://" + normalized_path
    if not default_extension.is_empty() and normalized_path.get_extension().is_empty():
        normalized_path += default_extension
    return normalized_path

func ensure_directory_for_path(res_path: String) -> bool:
    ...
    if make_dir_error != OK:
        printerr("Failed to create directory for path: " + res_path + " error=" + str(make_dir_error))
        return false
    return true

func generate_uid_for_resource(res_path: String) -> bool:
...
func build_script_text(extends_class: String, class_name: String) -> String:
    var lines: Array[String] = ["extends " + extends_class, ""]
...
func create_script(params):
    ...
    var script_path = to_res_path(str(params.script_path), ".gd")
    if script_path.is_empty():
        printerr("Invalid script_path")
        quit(1)

    var extends_class := "Node"
    if params.has("extends_class") and str(params.extends_class).strip_edges() != "":
        extends_class = str(params.extends_class).strip_edges()

    var class_name := ""
    if params.has("class_name"):
        class_name = str(params.class_name).strip_edges()

    var script_text := ""
    if params.has("template") and str(params.template).strip_edges() != "":
        script_text = str(params.template)
    else:
        script_text = build_script_text(extends_class, class_name)

    if not ensure_directory_for_path(script_path):
        quit(1)

    var script_file = FileAccess.open(script_path, FileAccess.WRITE)
    if not script_file:
        printerr("Failed to create script file: " + script_path)
        quit(1)
    script_file.store_string(script_text)
    script_file.close()

    var uid_generated = generate_uid_for_resource(script_path)
    var result = {
        "script": script_path,
        "uidFile": script_path + ".uid",
        "uidGenerated": uid_generated
    }
    print(JSON.stringify(result))

func create_scene_with_script(params):
    if not params.has("scene_path"):
        printerr("scene_path is required")
        quit(1)

    var scene_path = to_res_path(str(params.scene_path), ".tscn")
    if scene_path.is_empty():
        printerr("Invalid scene_path")
        quit(1)

    var root_node_type := "Node2D"
    if params.has("root_node_type") and str(params.root_node_type).strip_edges() != "":
        root_node_type = str(params.root_node_type).strip_edges()

    var root_name := "root"
    if params.has("root_node_name") and str(params.root_node_name).strip_edges() != "":
        root_name = str(params.root_node_name).strip_edges()

    var script_path := ""
    var use_script := false
    if params.has("script_path") and str(params.script_path).strip_edges() != "":
        script_path = to_res_path(str(params.script_path), ".gd")
        use_script = true

    if use_script and not FileAccess.file_exists(script_path):
        var create_missing := false
        if params.has("create_script_if_missing"):
            create_missing = bool(params.create_script_if_missing)
        if create_missing:
            var extends_for_script := root_node_type
            if params.has("extends_class") and str(params.extends_class).strip_edges() != "":
                extends_for_script = str(params.extends_class).strip_edges()
            var class_name := ""
            if params.has("class_name"):
                class_name = str(params.class_name).strip_edges()
            var default_script_text = build_script_text(extends_for_script, class_name)
            if not ensure_directory_for_path(script_path):
                quit(1)
            var new_script = FileAccess.open(script_path, FileAccess.WRITE)
            if not new_script:
                printerr("Failed to create missing script: " + script_path)
                quit(1)
            new_script.store_string(default_script_text)
            new_script.close()
            generate_uid_for_resource(script_path)
        else:
    ...
    if not scene_root:
        printerr("Failed to instantiate root node type: " + root_node_type)
        quit(1)
    ...
    if use_script:
        ...
        if not loaded_script:
            ...
            quit(1)
        scene_root.set_script(loaded_script)

    if not ensure_directory_for_path(scene_path):
        quit(1)
    ...
    generate_uid_for_resource(scene_path)
    if use_script:
        generate_uid_for_resource(script_path)

    var result = {
        "scene": scene_path,
        "script": script_path if use_script else "",
        "scriptUidFile": (script_path + ".uid") if use_script else "",
        "rootNodeType": root_node_type,
        "rootNodeName": root_name
    }
    print(JSON.stringify(result))
