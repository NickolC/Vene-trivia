...
// Load GODOT_PATH (and other vars) from the repo-root .env.
// build/index.js lives at <repo>/godot-mcp/build/index.js, so .env is two levels up.
...
class GodotServer {
  ...
  private parameterMappings: Record<string, string> = {
    ...
    'create_script_if_missing': 'createScriptIfMissing',
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        {
          name: 'create_script',
          description: 'Create a GDScript file and generate its .uid sidecar',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: {
              ...
              extendsClass: {
                type: 'string',
                description: 'Optional: base class to extend (e.g., Node2D, Control)',
              },
              className: {
                type: 'string',
                description: 'Optional: class_name to declare inside the script',
              },
              template: {
                type: 'string',
                description: 'Optional: full custom script content',
              },
            },
        ...
        {
          name: 'create_scene_with_script',
          description: 'Create a .tscn and optionally attach/create a .gd with UIDs',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: {
              ...
              rootNodeType: {
                type: 'string',
                description: 'Optional root node type (default: Node2D)',
              },
              rootNodeName: {
                type: 'string',
                description: 'Optional root node name (default: root)',
              },
              scriptPath: {
                type: 'string',
                description: 'Optional script path to attach to the root node',
              },
              createScriptIfMissing: {
                type: 'boolean',
                description: 'When true, create scriptPath automatically if missing',
              },
              extendsClass: {
                type: 'string',
                description: 'Optional script extends class (when creating missing script)',
              },
              className: {
                type: 'string',
                description: 'Optional class_name for auto-created script',
              },
            },
            required: ['projectPath', 'scenePath'],
          },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (request.params.name) {
        ...
        case 'create_scene_with_script':
          return await this.handleCreateSceneWithScript(request.params.arguments);
        case 'add_node':
  ...
  private async handleCreateScript(args: any) {
    ...
    if (!args.projectPath || !args.scriptPath) {
      return this.createErrorResponse(
        ...
        ['Provide valid paths for both the project and the script']
      );
    }
    ...
    if (args.extendsClass && !this.validateClassName(args.extendsClass)) {
      return this.createErrorResponse(
        'Invalid extendsClass',
        ['extendsClass must be a Godot class name (no paths, no file extensions)']
      );
    }

    if (args.className && !this.validateClassName(args.className)) {
      return this.createErrorResponse(
        'Invalid className',
        ['className must be a valid GDScript class identifier']
      );
    }
    ...
    try {
      ...
      if (args.extendsClass) params.extendsClass = args.extendsClass;
      if (args.className) params.className = args.className;
      if (typeof args.template === 'string') params.template = args.template;
      ...
      if (stderr && (stderr.includes('[ERROR]') || stderr.includes('Failed'))) {
        return this.createErrorResponse(
          ...
          [
            'Check if scriptPath is valid and writable',
            'Use a valid Godot base class in extendsClass',
            'Verify className is a valid identifier',
          ]
        );
      }
  ...
  /**
   * Handle the create_scene_with_script tool
   */
  private async handleCreateSceneWithScript(args: any) {
    args = this.normalizeParameters(args);
    ...
    try {
      ...
      if (args.rootNodeName) params.rootNodeName = args.rootNodeName;
      if (args.scriptPath) params.scriptPath = args.scriptPath;
      if (typeof args.createScriptIfMissing === 'boolean') {
        params.createScriptIfMissing = args.createScriptIfMissing;
      }
      if (args.extendsClass) params.extendsClass = args.extendsClass;
      if (args.className) params.className = args.className;

      const { stdout, stderr } = await this.executeOperation('create_scene_with_script', params, args.projectPath);

      if (stderr && (stderr.includes('[ERROR]') || stderr.includes('Failed'))) {
        return this.createErrorResponse(
          `Failed to create scene with script: ${stderr}`,
          [
            'Check if scenePath/scriptPath are valid and writable',
            'Use valid Godot classes for rootNodeType/extendsClass',
            'Set createScriptIfMissing to true when scriptPath does not exist',
          ]
        );
      }
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Scene scaffold created successfully at: ${args.scenePath}\n\nOutput: ${stdout}`,
          },
        ],
      };
    } catch (error: any) {
      return this.createErrorResponse(
        `Failed to create scene with script: ${error?.message || 'Unknown error'}`,
        [
          'Ensure Godot is installed correctly',
          'Check if the GODOT_PATH environment variable is set correctly',
          'Verify the project path is accessible',
