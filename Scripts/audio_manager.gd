...
const LEVEL1_MUSIC_PATH := "res://Music/Nivel1/Caracas_High_Score.mp3"
...
var _level1_music_stream: AudioStream
...
func _setup_players() -> void:
	...
	_level1_music_stream = _load_looped_stream(LEVEL1_MUSIC_PATH)
...
func _handle_scene_audio(scene_path: String, scene_script_path: String) -> void:
	...
	if _is_any_level_scene(scene_path, scene_script_path):
		...
		_stop_music()
		if _is_level1_scene(scene_path, scene_script_path):
			_debug_log("Detected level 1 scene")
			_play_transition_then_level1(token)
		else:
...
func _is_level1_scene(scene_path: String, scene_script_path: String) -> bool:
	if not scene_path.is_empty():
		if LEVEL1_SCENES.has(scene_path):
			return true

		var scene_file_name := scene_path.get_file().to_lower()
		if scene_file_name == "nivel1.tscn" or scene_file_name == "nivel 1.tscn":
			return true

	if scene_script_path.is_empty():
		return false

	return scene_script_path.get_file().to_lower() == "nivel1.gd"
...
func _play_music(path: String) -> void:
	...
	elif path == LEVEL1_MUSIC_PATH:
		target_stream = _level1_music_stream
	else:
...
func _play_transition_then_level1(token: int) -> void:
	if _transition_sfx_player == null or _transition_sfx_player.stream == null:
		_debug_log("Transition stream missing; fallback to level 1 music")
		_play_music(LEVEL1_MUSIC_PATH)
		return

	_debug_log("Play transition SFX then level 1")
